...
describe("checkNoVerbatimSourceValues (hole 1 / D38)", () => {
  it("flags a ja value byte-identical to its en counterpart — the exact hole-1 mutation reproduced (setting settings.general.providers.title to the literal English 'Providers')", () => {
...
describe("checkSourceLocalePluralCompleteness (hole 2 / D39)", () => {
  it("flags an EN plural family missing _one while _other survives — the exact hole-2 mutation reproduced (deleting modelCount_one from en/settings.json)", () => {
  ...
  it("does not flag a family with no _other member at all — derivePluralFamilyBases never recognizes it as a family, so it is invisible to this function by construction; checkPluralSiblings already reports that case and this function must not regress it", () => {
    ...
    expect(checkSourceLocalePluralCompleteness(catalog, "en")).toEqual([]);
    // the pre-existing check that DOES catch this shape:
  ...
  it("requires categories derived from Intl.PluralRules rather than a hardcoded ['one','other'] literal — proven by requiring a category no literal would guess", () => {
    // "en-US" only ever needs {one, other}, so use a locale whose CLDR set is
    // richer to prove the requirement list is not a fixed two-item literal.
    // Polish needs {one, few, many, other}; this is not a registered locale,
    // but pluralCategoriesForLocale takes an intlTag straight from Intl, so we
    // can call the underlying primitive directly to prove the same code path.
    const plCategories = new Intl.PluralRules("pl-PL").resolvedOptions().pluralCategories;
    expect(plCategories.length).toBeGreaterThan(2); // sanity: richer than en's 2
  });
});
