...
const CopyButton: React.FC<{
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={`${className} cursor-pointer ${positionClass}`}
    >
      <svg fill="#fff" viewBox="0 0 24 24" className="size-6 md:size-8">
      ...
      {copied && (
        <span className="text-green-600 font-semibold text-xs absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full">
          Copied!
        </span>
      )}
    </button>
