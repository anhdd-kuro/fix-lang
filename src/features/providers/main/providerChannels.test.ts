 ...
 * `ipcMain.handle` and invoked directly. `~/features/providers/shared/providers` and
 * `~/features/providers/shared/modelRef` are deliberately left unmocked — they are the logic
...
vi.mock("~/features/providers/store/profileSecretStore", async () => {
  const { PROVIDER_SUPPORTS_API_KEY, PROVIDER_SUPPORTS_PROVISIONING_KEY } =
    await import("~/features/providers/shared/providers");
  return {
...
import type { SecretKind } from "~/features/providers/store/profileSecretStore";
