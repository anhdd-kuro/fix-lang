 ...
 * the window grows to a single fixed 26x26 / +10 size used for every mode
...
describe("overlayWindow — O2 fixed 26x26 / +10 size for every mode", () => {
  beforeEach(() => {
    destroyOverlayWindow();
    lastWindow = null;
    vi.clearAllMocks();
  ...
  it("creates the overlay window at 26x26, not the old 20x20 spinner-only size", () => {
    createOverlayWindow();

    expect(electronMocks.browserWindowCtor).toHaveBeenCalledWith(
      expect.objectContaining({ width: 26, height: 26 }),
    );
  });
