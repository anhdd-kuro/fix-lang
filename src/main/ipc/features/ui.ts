...
export const registerUiHandlers = () => {
  ...
  // Clipboard handlers
  ipcMain.handle("get-clipboard-text", async (_event: Electron.IpcMainInvokeEvent) => {
    try {
  ...
  ipcMain.handle("set-clipboard-text", async (_event: Electron.IpcMainInvokeEvent, text: string) => {
    try {
  ...
  // Dialog handlers
  ipcMain.handle("show-message-box", async (_event: Electron.IpcMainInvokeEvent, options: Electron.MessageBoxOptions) => {
    try {
      const mainWindow = getMainWindow();
      if (!mainWindow) {
  ...
  // External links
  ipcMain.handle("open-external-link", async (_event: Electron.IpcMainInvokeEvent, url: string) => {
    try {
  ...
  // Window handling
  ipcMain.handle("get-current-window-id", (_event: Electron.IpcMainInvokeEvent) => {
    try {
