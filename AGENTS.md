...
- **Analytics** — Overview dashboard: stat cards, preset donut/time-series charts (`PresetWeightChart`), token activity calendar; shared All/30d/7d range with Models tab (`RANGE_AWARE_TABS` in `MainWindow/App.tsx`). Dashboard tabs: overview, history, models, usage, logs, about (`MainWindow/dashboardTabs.ts`).
