...
- **Autocomplete** — ghost-text suggestions in the Ask AI input window only: once the prefix reaches 12 characters and a debounce elapses, a greyed continuation appears at the caret. **Tab** accepts it into the textarea; Tab over a selection is refused. **The first Esc clears the ghost and leaves the window open; a second Esc cancels the window.** Enter always submits the typed question, never an unaccepted ghost. Rendered through a plain mirror `<span>` — never `MarkdownView`, never `dangerouslySetInnerHTML` — because it is untrusted model output landing in an input surface. A suggestion belongs to one exact `{text, caret}` state; moving the caret invalidates it, enforced at the points that cost something (before the billed request, before it paints, and on accept) rather than on any one DOM event. Settings → General gets an enable toggle and a model picker (the `""` sentinel shows as "Same as Ask AI"), plus a privacy hint recommending a local provider (Ollama or LM Studio), since it sends text the user is still typing. A seventh dashboard tab, Autocomplete, sits second-to-last (before About) reading per-day rollups: today, month-to-date, the 62-day series, and how much of the 1500/day request cap is spent. `requests` counts attempts DISPATCHED, not completions — an aborted request still bills, and the daily cap is the only hard stop against an overnight bill. `estimatedCostUsd` sums PRICED responses only, and a day's `responses`/`tokenlessResponses`/`unpricedResponses` counters must agree with its sums or a month total shows money no day can account for. No SQLite history rows are written — only the aggregates — and there is no build-time feature tag; the runtime `enabled` toggle in Settings is the off-switch.
