...
- **App shell**
  ...
  - **Pre-release channel** — a Pre-release section below the stable flow in About → Updates, with its own check button and its own result line, visible to every user. Discovery is button-only, so a routine update check is unchanged. Switching to a beta asks a main-process confirm naming the exact version; that dialog states the mechanics (quit → Homebrew download → reopen, and that reverting needs no confirm) **and exactly one risk**: settings a beta writes may not be readable by stable afterwards, in which case the stored config is discarded on the next stable launch and reverting does not restore it (`settings.updates.prerelease.confirm.configWarning`, appended to the detail by `buildPrereleaseConfirmDetail` in `src/main/update/index.ts`). It warns about nothing beyond those two — do not credit it with more. **Revert to stable asks nothing** — it is the safe direction, reached for precisely when a beta is misbehaving. Both run a detached Homebrew helper that fetches, uninstalls the current cask token and installs the other, restoring the original if that fails; its output goes to `userData/logs/homebrew-channel-switch.log`, NOT the stable upgrade's `homebrew-update.log`. The switch never passes `--zap` or `--force` and neither cask has a `zap` stanza, so settings, profiles, keys and history are carried across **both directions untouched** — that is a claim about the switch, not about compatibility. Whether a *stable* build can still read what a *beta* wrote is a separate question with no guarantee behind it: stored config only ever migrates forward (`configVersion` in `apiStore.ts` gates one-shot migrations and has no inverse), and every `electron-store` here is built with `clearInvalidConfig: true`, so one value failing the older build's ajv schema wipes that whole store file. Keep the two claims apart when editing these docs. Betas are published from `beta/**` branches by `.github/workflows/prerelease.yml` under the same gates as a stable release. See [Pre-release channel](.claude/skills/fixlang/fixlang-prerelease-channel/SKILL.md).
...
- **The pre-release check is a different GitHub endpoint** — `/releases/latest` never returns a prerelease, so discovery pages the release-list endpoint and validates the array item by item, dropping bad items rather than failing the batch. It runs **only** on the explicit button press, so an ordinary check still costs one unauthenticated request. The `Link` header is untrusted: paging follows it only when origin+pathname equal the endpoint exactly. Version ordering comes from `src/main/update/prereleaseVersion.ts` (`X.Y.Z-beta.N`, nothing else); `parseStableVersion` stays module-private so the two disjoint grammars cannot be confused. The never-delete tap-lag gate deliberately does **not** use it: `runInstallUpdate` parses both `offered` and `target` with `parseCurrentVersion` (`parseStableVersion(v) ?? parsePrereleaseVersion(v)`), which is why the gate keeps working on the beta channel instead of collapsing to `null` there — see the comment at `updateService.ts:1350-1354`. Swapping either side back to `parseStableVersion` is what would silently disable it for beta users, so leave that call alone.
- **The marker distinguishes a rollback from a success** — `pending-update.json` carries the **target** `caskToken` plus an optional `fromCaskToken`, and reconcile probes the target token's Caskroom path. A new `"rolled-back"` outcome fires only when `currentVersion !== fromVersion`, because mid-flight the source cask is legitimately still installed; without it, a revert whose restore reinstalled a *newer* beta than the user had reported as a completed install. A revert's marker is token-identical to an ordinary stable upgrade (both target `fixlang`), so routing must also consult `fromVersion`.
