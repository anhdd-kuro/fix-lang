...
- **Source-app context** — Transform and PromptGen append the frontmost app name ("Slack", "Mail") — as a `# Metadata context` block — to the **system prompt** (`src/main/ai.request/transform-context.ts`), so the model can match that app's register; it trails rather than leads the preset's own instructions so the preset's text stays a stable, cacheable prefix — a different active app now only varies the uncached suffix instead of busting the provider's prompt cache for the whole request on every app change (`./cache-strategy`). The trade-off is the reverse of a leading block: trailing metadata carries less weight against the preset's own instructions, since models generally weight later-vs-earlier context differently and the app hint no longer sits first. Best-effort: dropped entirely when the read fails or FixLang itself is frontmost, leaving the system prompt byte-identical. Every read is logged under scope `accessibility.activeApp` (debug on read/drop, warn on failure). The block carries an `AppContextFormattingPolicy` (`"preserve-input-markup" | "adapt-to-app"`), resolved per preset by the pure `appContextPolicyForPreset(presetId)`; only the `structured-text` built-in gets `adapt-to-app`, and every other preset plus PromptGen keep the default `preserve-input-markup` variant — whose wording is byte-identical to the pre-policy block, which is load-bearing (it is what keeps every other preset's and PromptGen's prompt unchanged) and is pinned by a literal-string test.
- **Presets** — built-in Correction, Summarize, Translate, Prompt optimization, Business Writing, Context-Aware Structured Text, Ask AI; each preset has its own hotkey, model, system prompt, and generic reasoning effort (`none`/`low`/`medium`/`high`). Retired values are NOT merely deleted from `REASONING_EFFORT_STEPS` — `RETIRED_EFFORTS` in `src/shared/reasoningEffort.ts` maps stored `minimal` to `low` and stored `xhigh` ("Maximum") to `high`, and `sanitizeReasoningEffort`, `reasoningEffortToStepIndex`, and `reasoningForAiSdk` all route through it. Dropping a retired value instead would read as `provider-default` downstream, which silently CHANGES a stored preset's behaviour, and `reasoningEffortToStepIndex` would land on -1 → None. Each preset also carries an optional per-preset `outputMode` (`"inherit" | "paste" | "popup"`, `CorrectionPreset.outputMode` in `src/stores/apiStore.ts`) that overrides the global Transform output mode at request time; `"inherit"` (and unset) defers to `outputModeStore`. Settings exposes it as a `Select` (Inherit/Paste/Popup) for every preset — so **both** delivery paths must resolve it (`correction.ts`'s ordinary hotkey branch and `askFlow.ts`), never read `outputModeStore` directly; reading the store in one of them leaves that Select visible, writable, persisted and inert for the six polish presets. **Ask AI** (`Control+Shift+A`) is the one built-in with `requiresInput: true`: instead of aborting on an empty selection, its hotkey opens a small input window (`askInputWindow.ts`) that carries the current selection as OPTIONAL context via `getHighlightedTextForOptionalContext()`, sends one shot through `askFlow.ts`, and renders the answer as GFM markdown in a capped, cascading multi-instance popup (`askResultWindow.ts`, cap 5). Its `markdownOutput: true` field is only meaningful — and only shown as a Settings control — on a `requiresInput` preset.
- **The Ask AI selection read and its answer are both untrusted** — three guards that each look optional and are not. (1) `getHighlightedTextForOptionalContext()` reports `""` when the Cmd-C keystroke did not change the clipboard, because with nothing selected the keystroke is a no-op and `clipboard.readText()` afterwards hands back whatever was there before (a password, say) as if it were a selection; the guard polls the clipboard for a raw CHANGE (`waitForClipboardChange`, `src/utils.ts`) rather than the trimmed-string compare it used to do — AppleScript no longer returns clipboard text at all (see the correction hotkey's combined frontmost-app-read-then-copy, `getHighlightedTextWithActiveApp`), so there is no `stdout.trim()` asymmetry left to silently disable the guard for exactly the values it protects. (2) A pending Ask input must not outlive its profile: `runAskFlow` captured profile A's preset but `fixGrammar(message, preset.id)` re-resolves that id at SUBMIT time, so a mid-typing profile switch would send A's context through B's model, provider and key. Every profile activation therefore goes through `notifyActiveProfileChanged()` (`src/main/profileChange.ts`), which dismisses the input before broadcasting `ACTIVE_PROFILE_CHANGED`; broadcasting that channel directly is forbidden and a `profileChange.test.ts` source guard fails on it. (3) `MarkdownView.tsx` renders MODEL-CONTROLLED markdown, so `img: () => null` (an `<img>` fetches its URL on mount with no click — a read receipt, or exfiltration once an injected answer encodes the selection into the path) and links route through `openExternalLink` rather than `target="_blank"`, which would get Electron's default window-open behaviour: an unmanaged app-owned window outside the result-window cap, with a preload attached.
