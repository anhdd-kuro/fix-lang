...
- **Ask & autocomplete**
  ...
  - **Request context** — resolved once per press (`askEnvironment.ts`): app locale, macOS system language, active keyboard input source (best-effort, absent when unreadable), current local time, and the last 5 transforms as **preset names and timestamps only** — never their text. Rendered by `buildAskDirectives` and applied to system prompts by `withUserMetadata` (`ai.request/user-metadata.ts`) for every preset and Autocomplete; Ask still appends the same string to the submitted user message. Empty/absent directives leave the prompt byte-identical.
