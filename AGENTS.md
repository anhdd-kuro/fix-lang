...
- **Ask AI** — the one preset that needs no selection: its hotkey opens a small input window, treats any current selection as optional context, and answers in a cascading popup (up to 5 at once) that reads selection → question → answer. Selection and question are plain text clamped to 3 lines behind a fold; only the answer is GFM markdown, and it is never clamped. Nothing selected means no selection block at all.
