...
- **The ⌘C fallback can lie, and is guarded three ways** — the AppleScript ends in `return the clipboard`, so an app that *swallows* the synthesized ⌘C hands back the user's PREVIOUS clipboard, which used to reach a provider as if it were the selection; `getHighlightedText` now compares against its own pre-copy snapshot and resolves `""` so `resolveSelectedText`'s `clipboardEmpty` branch refuses. Cost: a selection identical to what the user just copied refuses instead of transforming — deliberate, since a visible refusal is recoverable and wrong-text-to-a-provider is not. The `finally` restore is skipped when `clipboard.readText()` was `""` while `availableFormats()` was non-empty, because `writeText("")` **blanks** an image-only or RTF-only pasteboard. And `withHotkeyThrottle` now carries a per-accelerator **in-flight set** on top of `HOTKEY_THROTTLE_MS`: `validateHotkeys` keeps no reserved-system-key list, so a user can bind ⌘C, and the AX read's 1500 ms timeout can push the synthesized copy outside the 500 ms window — time alone cannot prevent that recursion. Never log the selection: `src/main/index.ts` patches `console.*` into `~/.fixlang/log/`, outside `userData` and reached by **neither** `redactLogMessage` nor `redactLogContext`. Lengths and booleans through `logger` are fine.
