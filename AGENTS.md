...
- **Selection read** — every hotkey reads the selection the same way: snapshot the clipboard, **empty it**, send Cmd-C, poll the pasteboard for content, restore the snapshot in `finally`. The emptying is the load-bearing step. Polling for a *change* cannot tell "nothing was selected" from "the selection is byte-identical to what was already on the clipboard", and the second is not exotic — copying text and leaving it selected is an ordinary way to arrive at a hotkey. That ambiguity cost both paths something real: the strict path (`getHighlightedText`, `getHighlightedTextWithActiveApp`) papered over it by falling back to the PREVIOUS clipboard, so nothing-selected silently transformed a stale, unrelated clipboard; Ask AI's `getHighlightedTextForOptionalContext` refused that fallback — a stale clipboard reaching a model as if it were the selection is a leak, and it has no "no text selected" abort to catch one — and paid in false negatives instead, dropping the context block for a selection it could not distinguish from nothing. Against an empty pasteboard both questions collapse into one, so **`""` now means exactly "the copy produced nothing"** and no caller needs a fallback. Cost, stated: for the length of one read the user's clipboard is empty, and a hard crash inside that window leaves it empty rather than restored (`finally` covers every non-crash path, throws included). Nothing else is lost — the restore has always been `writeText`, which already discarded non-text flavours. **The log key is `selectionChanged`, never `clipboardChanged`**: `redactLogContext` blanks any key merely *containing* `clipboard`, so the obvious name persisted as `"[REDACTED]"` and the one field that could confirm this diagnosis was blank in the logs that showed the bug. Same trap as the `selectionPoll` latency phase. Ask AI logs its own `Ask context resolved` line (`contextLength`/`contextAttached`, lengths only) because without it "the user selected nothing" and "the read dropped their selection" both look like an input window with no context card.
