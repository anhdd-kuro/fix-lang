...
- **Selection capture is AX-first, ⌘C second** — `resolveSelectedText` (`src/main/keybindings/selectionSource.ts`, pure + DI) asks `getAxSelectedText` (`src/main/accessibility/selectedText.ts`) for `kAXSelectedText` on the focused element through the same osascript/System Events channel `getActiveApp()` uses, and falls back to the ⌘C clipboard read only when that misses AND the user's `clipboardFallbackStore` setting (Settings > General, **default ON**; `shared/clipboardFallback.ts` — only an explicit `false` disables) allows it. Both reads must stay AFTER `getActiveApp()` and BEFORE `showOverlaySpinner()`: once a FixLang window is up, `first application process whose frontmost is true` is FixLang and the AX read reports our own empty selection. Order inside the decision table is load-bearing — **`secure` is checked FIRST**, before the non-blank test, so a future producer bug that paired `status: "secure"` with non-empty text still could not route a password field to ⌘C. Probed against real targets: a focused `NSSecureTextField`, WebKit `type=password` and Chromium `type=password` all report role `AXTextField` with **subrole** `AXSecureTextField`, which is why the script reads `AXSubrole` too; `AXPasswordField` does not exist in macOS at all. `permissionDenied` throws `AccessibilityPermissionError` only when the fallback is OFF (with it ON the ⌘C attempt surfaces the same denial itself), and `axPermission.ts`'s code regex is **anchored to `\((?:-1743|-25211)\)`** because `error.message` embeds the whole script — an unanchored match made every exec failure look like a denial. `-1728` is attribute-missing, never permission.
- **The ⌘C fallback can lie, and is guarded three ways** — the AppleScript ends in `return the clipboard`, so an app that *swallows* the synthesized ⌘C hands back the user's PREVIOUS clipboard, which used to reach a provider as if it were the selection. Two checks, because either alone is blind somewhere: `getHighlightedText` compares against its own pre-copy snapshot, AND re-reads the pasteboard afterwards — on a NON-TEXT pasteboard the snapshot is already `""`, so it can never match, and `return the clipboard` there prints AppleScript's raw-data literal (`«data PNGf8…»`), i.e. megabytes of hex that would otherwise be accepted as the selection. Either check failing resolves `""` so `resolveSelectedText`'s `clipboardEmpty` branch refuses. Cost: a selection identical to what the user just copied refuses instead of transforming — deliberate, since a visible refusal is recoverable and wrong-text-to-a-provider is not. The `finally` restore is gated on **"did the copy actually change the pasteboard"**, NOT on classifying `availableFormats()`: `writeText("")` itself declares an *empty text flavour*, so a format-based guard counted a pasteboard FixLang had already restored as non-text, skipped the next run's restore, and left the user's selection sitting there. A copy that never landed has nothing to undo, so a failed or swallowed ⌘C now leaves an image/RTF clipboard entirely untouched. And `withHotkeyThrottle` carries a per-accelerator **in-flight map** on top of `HOTKEY_THROTTLE_MS`: `validateHotkeys` keeps no reserved-system-key list, so a user can bind ⌘C, and the AX read's 1500 ms timeout can push the synthesized copy outside the 500 ms window — time alone cannot prevent that recursion. It yields after `HOTKEY_IN_FLIGHT_STALE_MS` (5 min) because nothing it fences is guaranteed to settle (no provider request sets a timeout or `AbortSignal`), and release is identity-checked so a stale run finishing late cannot clear the press that superseded it. Never log the selection: `src/main/index.ts` patches `console.*` into `~/.fixlang/log/`, outside `userData` and reached by **neither** `redactLogMessage` nor `redactLogContext`. Lengths and booleans through `logger` are fine.
- **Source-app context** — Transform and PromptGen prepend the frontmost app name ("Slack", "Mail") — as a `# Metadata context` block — to the **system prompt** (`src/main/ai.request/transform-context.ts`), so the model can match that app's register; it leads rather than trails the preset's own instructions so it carries more weight, at the cost of busting the provider's prompt cache on every app change instead of only appending an uncached suffix (`./cache-strategy`). Best-effort: dropped entirely when the read fails or FixLang itself is frontmost, leaving the system prompt byte-identical. Every read is logged under scope `accessibility.activeApp` (debug on read/drop, warn on failure). The block carries an `AppContextFormattingPolicy` (`"preserve-input-markup" | "adapt-to-app"`), resolved per preset by the pure `appContextPolicyForPreset(presetId)`; only the `structured-text` built-in gets `adapt-to-app`, and every other preset plus PromptGen keep the default `preserve-input-markup` variant — whose wording is byte-identical to the pre-policy block, which is load-bearing (it is what keeps every other preset's and PromptGen's prompt unchanged) and is pinned by a literal-string test.
- **Presets** — built-in Correction, Summarize, Translate, Prompt optimization, Business Writing, Context-Aware Structured Text; each preset has its own hotkey, model, system prompt, and generic reasoning effort (`none`/`low`/`medium`/`high`). Retired values are NOT merely deleted from `REASONING_EFFORT_STEPS` — `RETIRED_EFFORTS` in `src/shared/reasoningEffort.ts` maps stored `minimal` to `low` and stored `xhigh` ("Maximum") to `high`, and `sanitizeReasoningEffort`, `reasoningEffortToStepIndex`, and `reasoningForAiSdk` all route through it. Dropping a retired value instead would read as `provider-default` downstream, which silently CHANGES a stored preset's behaviour, and `reasoningEffortToStepIndex` would land on -1 → None.
...
- **Tray Providers card** — `TrayWindow/components/TrayProviderSummary.tsx` replaces the old single OpenRouter-credit card: one tab per provider, reusing the Usage tab's own `buildUsageSubTabs` so the two surfaces cannot disagree about which providers have an account. Only the ACTIVE tab's body mounts — that is what keeps opening the tray to one request instead of one per provider. **Which OpenAI project is not discoverable**: an admin key is organization-scoped and OpenAI exposes no endpoint naming the project behind a key, so `SettingsStore.openaiProjectId` is a user-supplied setting (validated by `shared/openaiProject.ts`, submitted through `connect-provider`, cleared on profile export as account-local detail). A project absent from `projectCosts.projects` renders "no spend", never `$0.00` — `parseProjectCosts` drops zero-cost projects, so absent also covers "unknown project", and a dollar figure would assert something `/costs` never said.
- **Two tray siblings keyed by `profileId` must carry DISTINCT key prefixes** (`TrayWindowMain.tsx`). `useActiveProfileId` returns `""` until its IPC resolves; two siblings sharing the key `""` collide in React's reconciliation map (keyed by `key`, so the second evicts the first), and when the key flips to the real id the evicted fiber is never deleted — its DOM node stays behind as a **duplicate card**. Production React logs no duplicate-key warning, so nothing surfaces it but a rendered count: `TrayWindowMain.test.ts` asserts one card and pins the distinct-prefix arrangement.
