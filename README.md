...
- **Caveman** compresses text instead of polishing it — drops articles, filler and hedging, keeps technical terms exact and code blocks untouched. Three intensities in Settings → Transform: **Lite** (trim filler, stay professional), **Full** (drop articles, fragments allowed), **Ultra** (maximum compression, arrows for cause and effect). The intensity control appears only on Caveman
...
- **Perfect prompt** ships built in — Correction → Prompt optimization → Caveman, waiting in Settings → Combos. It arrives with **no hotkey**: assign one there before it can run, since a single press is three AI calls. Edit it freely; delete it and it comes back on the next load, the same as a built-in preset
