...
- **Your clipboard is left alone in most apps** — FixLang reads your selection through the macOS Accessibility API, so in apps that support it (TextEdit, Mail, Notes, Safari, Xcode, most native apps) nothing is copied and your clipboard is never touched. Apps that don't expose their text — terminals (Ghostty, Warp, Kitty), Zed, Sublime Text, Google Docs, Slack, Notion, VS Code, and anything inside a VM or remote desktop — fall back to pressing ⌘C, then FixLang puts your previous clipboard back. Turn the fallback off in **Settings → General → "Use the clipboard when direct access fails"** to stop that entirely; transforms then simply do nothing in those apps. Two honest limits: only *text* can be restored, so a clipboard holding an image or rich text is replaced by the ⌘C and cannot be put back; and **Direct paste** output still writes to your clipboard to deliver the result, whatever this setting says
- **Password fields are refused, not transformed** — a focused secure text field is detected and skipped outright, and no ⌘C is sent into it
